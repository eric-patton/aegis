...
This document is the synthesized design. The audit trail with rationale and rejected alternatives lives in `decisions.md` (D-001 through D-126); the research behind it lives in `../research/`.
...
- **Death's Toll**: a visible meter that fills on death and drains over time. Routine deaths never scar. Clustered or boss-tier deaths risk a scar: a lost eye, a crushed hand, a haunted look. A handful per playthrough at most, each with a costly path back to parity, each a dialogue hook. NPCs who watch you fall remember. *Shipped whole (D-098): the deterministic ledger, the three scars matched to their deaths, each one's costly cure road (the stillroom's knife, the smith's brace, the songhall's laying), and the stead noticing the marks; the catalog and tier-scaled fill grow from here.*
